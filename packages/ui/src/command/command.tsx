...
// Direct lucide imports, matching this file's existing glyph. Reaching for the
// `Icons` facade instead would pull the whole iconography registry (a
// non-tree-shakeable object literal, plus @iconify/react) into every consumer
// of `@vita/ui/command` — including the website's docs-search island, which
// deliberately stays off that abstraction.
...
// Private: only `CommandDialogPopup` composes these. Re-exporting invites
// consumers to bypass the popup and widens the API surface unnecessarily.
...
function CommandDialogBackdrop({
  ...
  return (
    <CommandDialogPrimitive.Backdrop
      className={cn(
        'fixed inset-0 z-(--z-layer-system) bg-black/32 backdrop-blur-sm transition-all duration-200 data-ending-style:opacity-0 data-starting-style:opacity-0',
        className
      )}
...
function CommandDialogViewport({
  ...
  return (
    <CommandDialogPrimitive.Viewport
      className={cn(
        'pointer-events-none fixed inset-0 z-(--z-layer-system) flex flex-col items-center overflow-hidden px-4 py-[max(--spacing(4),4vh)] sm:py-[10vh]',
        className
      )}
    ...
    <CommandDialogPortal {...portalProps}>
      ...
      <CommandDialogViewport>
        <CommandDialogPrimitive.Popup
          className={cn(
            'pointer-events-auto relative row-start-2 flex max-h-[calc(100dvh-2rem)] min-h-0 w-full min-w-0 max-w-xl -translate-y-[calc(1.25rem*var(--nested-dialogs))] scale-[calc(1-0.1*var(--nested-dialogs))] flex-col rounded-2xl border bg-popover not-dark:bg-clip-padding text-popover-foreground opacity-[calc(1-0.1*var(--nested-dialogs))] shadow-lg/5 outline-none transition-[scale,opacity,translate] duration-200 ease-in-out will-change-transform before:pointer-events-none before:absolute before:inset-0 before:rounded-[calc(var(--radius-2xl)-1px)] before:bg-muted/72 before:shadow-[0_1px_--theme(--color-black/4%)] data-nested:data-ending-style:translate-y-8 data-nested:data-starting-style:translate-y-8 data-nested-dialog-open:origin-top data-ending-style:scale-98 data-starting-style:scale-98 data-ending-style:opacity-0 data-starting-style:opacity-0 **:data-[slot=scroll-area-viewport]:data-has-overflow-y:pe-1 **:data-[slot=scroll-area-viewport]:overscroll-contain sm:max-h-105 dark:before:shadow-[0_-1px_--theme(--color-white/6%)]',
            className
          )}
...
export function CommandInput({
  ...
  /**
   * Swaps the search glyph for a spinner and marks the input busy. The glyph
   * and the spinner share the addon's fixed icon slot, so toggling this
   * shifts no layout.
   */
  loading?: boolean
  /**
   * Screen-reader copy announced while `loading`. The start-addon slot is
   * `aria-hidden`, so the spinner alone says nothing to assistive tech — pass
   * localized copy (this package carries no i18n dependency) to get a live
   * region alongside it.
   */
  loadingLabel?: string
}): React.ReactElement {
  return (
    <div className="flex items-center gap-2 px-2.5 py-1.5">
      <AutocompleteInput
        ...
        size="lg"
        startAddon={
          loading ? <Loader2Icon className="motion-safe:animate-spin" /> : <SearchIcon />
        }
        {...props}
      />
      {/* Mounted whether or not it is busy: a live region inserted with its
          text already in place is not reliably announced. */}
      {loadingLabel !== undefined && (
        <span className="sr-only" role="status">
          {loading ? loadingLabel : ''}
        </span>
      )}
      {endAddon && <div className="shrink-0">{endAddon}</div>}
    </div>
  )
}
...
export function CommandList({
  ...
  return (
    <AutocompleteList
      className={cn('not-empty:scroll-py-2 overflow-y-auto not-empty:p-2', className)}
...
export function CommandShortcut({
  ...
  return (
    <kbd
      className={cn(
        'ms-auto font-[family-name:system-ui] font-medium text-muted-foreground/72 text-xs tracking-widest',
        className
      )}
